...
**Accessibility color modes.** Colorblind-friendly (`data-cvd="on"`, Okabe–Ito blue/orange) and high-contrast (`data-contrast="high"`, black/white ≥7:1) both re-declare the hue tokens (`--brand`, `--link`, `--hero-*`, …) on the `.hue-default` element (the single standard hue on `<body>`; the retired alternate colorsets live in `.claude/design-artifacts/hue-archive.css`) at **equal specificity**, per theme. Because they tie on specificity, **source order breaks the tie**: the CVD blocks are written *before* the high-contrast blocks on purpose, so when both are on, high contrast wins. Preserve that ordering if you touch these blocks. The Marigold CTA (`--accent`) is already CVD-safe and is left untouched by CVD. Text size (`data-textsize`) does **not** zoom — it retunes the fluid-root knobs (`--root-min`/`--root-max`) only, so type and `--space-*`/`rlh` rhythm scale together; add new stops as `[data-textsize="NNN"] { --root-min: …; --root-max: …; }` and register the value in both the boot IIFE whitelist and the stepper's `TEXT_STEPS`.
